...
| **Unlock daily reward (skip ad)** | Claim MyZong's daily 'watch ad to claim' reward without watching the ad. Forces the rewarded-ad show methods to invoke the reward callback immediately — the same no-ad path the app already uses when offline — so the reward is granted with no ad shown. |

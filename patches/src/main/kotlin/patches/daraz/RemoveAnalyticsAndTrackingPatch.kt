...
val removeAnalyticsAndTrackingPatch = bytecodePatch(
    name = "Remove analytics & tracking (WIP)",
    description = "⚠️ WORK IN PROGRESS — not yet fully verified on-device; use at your own risk. " +
        "No-ops the analytics/ad/fingerprint egress in Daraz — Alibaba UserTrack clickstream, " +
        "AUDID/UTDID fingerprint upload, Behavix behavior collection, Meta/Facebook App Events + GAID, " +
        "Firebase Analytics, Zalo device-tracking, Motu crash/APM, and the UCWeb webview telemetry " +
        "uploaders — plus manifest auto-collection flags. Login, push (ACCS/FCM), eKYC and the app's " +
        "own commerce API are left intact. Known gaps: Firebase Analytics runs inside Google Play " +
        "Services (relies on the manifest flag, not a code stub), residual native UCWeb telemetry " +
        "needs a DNS/host block, and the split-APK install is still being validated.",
) {
    compatibleWith(COMPATIBILITY_DARAZ)
